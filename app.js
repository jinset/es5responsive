...
function init() {
  ...
  for (let i = 0; i < numberOptions; i++) {
    ...
    document
      ...
      .insertAdjacentHTML(
        "afterbegin",
        '<div class="col-md-6 col-lg-6 option"><div class="option-number"><div class="blue-circle"><span class="text-gold">' +
          showNumber +
          '</span></div></div> <div class="text-option"><h2 class="text-blue">Lorem Ipsum</h2><span>Lorem ipsum dolor sit amet, consectetur adipisicing elit, seddo eiusmod tempor incididunt ut labore et dolore magnaaliqua.</span> </div></div>'
      );
  }
...
function searchInit() {
  document.getElementById("text-search").textContent =
    "Search Any Email Address";
  document.getElementById("text-subtext").textContent =
...
function successSend(person) {
  ...
  person.phone_numbers.forEach(function(element) {
    document
      .getElementById("phone-numbers")
      .insertAdjacentHTML(
        "afterbegin",
        '<li class="text-blue number-list-li">' + element + "</li>"
      );
  });

  person.relatives.forEach(function(element) {
    document
      .getElementById("relatives")
      .insertAdjacentHTML(
        "afterbegin",
        '<li class="relatives-list-li">' + element + "</li>"
      );
  });
}
